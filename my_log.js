...
window.onerror = function _log_err(desc, page, line, chr) {
	if (localStorage.log) {
		var ERR_MSG = LOG_PROMPT + ' ERROR: "' + desc + '" in ' + page.split('/').reverse()[0] + ':' + line + ':' + chr;

		console.log('%c' + _getTimestamp() + ' ' + ERR_MSG, 'color: #FF2929');
		return true;
	}
}
...
function _log() {
	...
	if (localStorage.log) {
		...
		switch (arguments.length) {
			...
			case 2:
				if (typeof(arguments[1])!='string') var stuff = arguments[1];
				else var color = arguments[1];
				what = arguments[0];
				break;
